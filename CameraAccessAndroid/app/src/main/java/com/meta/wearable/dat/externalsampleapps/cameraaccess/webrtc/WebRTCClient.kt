...
class WebRTCClient(private val context: Context) {
    ...
    fun createOffer(onSuccess: (SessionDescription) -> Unit) {
        ...
        peerConnection?.createOffer(object : SdpObserver {
            override fun onCreateSuccess(sdp: SessionDescription?) {
                sdp?.let { offer ->
                    val cappedSdp = applyBitrateCap(offer)
    ...
    private fun applyBitrateCap(sdp: SessionDescription): SessionDescription {
        val lines = sdp.description.split("\r\n").toMutableList()
        val modified = mutableListOf<String>()
        for (line in lines) {
            modified.add(line)
            if (line.startsWith("m=video")) {
                modified.add("b=AS:${WebRTCConfig.VIDEO_BITRATE / 1000}")
            }
        }
        return SessionDescription(sdp.type, modified.joinToString("\r\n"))
    }

    private fun applyEncodingParameters() {
        peerConnection?.senders?.forEach { sender ->
            if (sender.track()?.kind() == "video") {
                val params = sender.parameters
                if (params.encodings.isNotEmpty()) {
                    params.encodings[0].maxBitrateBps = WebRTCConfig.VIDEO_BITRATE
                    params.encodings[0].maxFramerate = WebRTCConfig.VIDEO_FRAMERATE
                    sender.parameters = params
                    Log.d(TAG, "Applied bitrate cap: ${WebRTCConfig.VIDEO_BITRATE / 1000}kbps, ${WebRTCConfig.VIDEO_FRAMERATE}fps")
                }
            }
        }
    }
