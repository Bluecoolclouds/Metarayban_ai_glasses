...
sealed class SignalingMessage {
    ...
    data class GlassesMessage(val message: String) : SignalingMessage()
    data class Reminder(val message: String, val id: Int) : SignalingMessage()
    data class TTSState(val enabled: Boolean) : SignalingMessage()
    data class TwitchConfig(val rtmpUrl: String) : SignalingMessage()
    object VideoStreamStarted : SignalingMessage()
    object VideoStreamStopped : SignalingMessage()
}
...
class SignalingClient {
    ...
    fun sendTTSToggle(enabled: Boolean) {
        sendJSON(JSONObject().put("type", "tts_toggle").put("enabled", enabled))
    }

    fun sendVideoStreamStart(format: String) {
        sendJSON(JSONObject().put("type", "video_stream_start").put("format", format))
    }

    fun sendVideoStreamStop() {
        sendJSON(JSONObject().put("type", "video_stream_stop"))
    }
        ...
        try {
            ...
            when (type) {
                ...
                "glasses_message" -> {
                    ...
                    if (msg.isNotEmpty()) {
                        Log.d(TAG, "Glasses message received: $msg")
                        onMessageReceived?.invoke(SignalingMessage.GlassesMessage(msg))
                    }
                }
                "reminder" -> {
                    val msg = json.optString("message", "")
                    ...
                    if (msg.isNotEmpty()) {
                        Log.d(TAG, "Reminder received: $msg")
                        onMessageReceived?.invoke(SignalingMessage.Reminder(msg, id))
                    }
                }
                "tts_state" -> {
                    val enabled = json.optBoolean("enabled", false)
                    Log.d(TAG, "TTS state: $enabled")
                    onMessageReceived?.invoke(SignalingMessage.TTSState(enabled))
                }
                "twitch_config" -> {
                    val rtmpUrl = json.optString("rtmpUrl", "")
                    if (rtmpUrl.isNotEmpty()) {
                        Log.d(TAG, "Twitch config received: ${rtmpUrl.take(40)}...")
                        onMessageReceived?.invoke(SignalingMessage.TwitchConfig(rtmpUrl))
                    }
                }
                "video_stream_started" -> {
                    Log.d(TAG, "Video stream started on server")
                    onMessageReceived?.invoke(SignalingMessage.VideoStreamStarted)
                }
                "video_stream_stopped" -> {
                    Log.d(TAG, "Video stream stopped on server")
                    onMessageReceived?.invoke(SignalingMessage.VideoStreamStopped)
                }
                else -> {
