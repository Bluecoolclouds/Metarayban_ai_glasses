...
object WebRTCConfig {
    ...
    private val turnCredentialsURL: String
        get() {
            val wsUrl = signalingServerURL
            val httpUrl = wsUrl
                .replace("wss://", "https://")
                .replace("ws://", "http://")
                .trimEnd('/')
            return "$httpUrl/api/turn"
        }
    ...
    private suspend fun fetchTurnCredentials(): List<PeerConnection.IceServer> =
        withContext(Dispatchers.IO) {
            ...
            val url = turnCredentialsURL
            Log.d(TAG, "Fetching TURN credentials from: $url")
